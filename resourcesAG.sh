...
APIKEY=$(echo $VCAP_SERVICES | jq -r '.[] | select(.service=="'$SVC'") .credentials[0].apikey')

# STATUS=$(echo $VCAP_SERVICES | jq -e . >/dev/null 2>&1  | echo ${PIPESTATUS[1]})
# echo $STATUS

echo ""
[ $APIKEY == "null" ] && echo "!!! ERROR: Credentials set in \$VCAP_SERVICES are NOT VALID !!!" || echo "Credentials set in \$VCAP_SERVICES"
